...
bool WiFiMulti::addAP(const char* ssid, const char *passphrase)
{
    ...
    //edit here, check if the WiFi exsits before
    for(uint8_t i = 0; i < APlist.size(); i++) 
    {
        WifiAPlist_t entry = APlist[i];
        if(String(entry.ssid) == String(ssid))
        {
            log_i("[WIFI][APlistAdd] WIFI exists before");
            // Serial.print(ssid);
            // Serial.println(" is duplicated");
            entry.ssid = strdup(passphrase);
            return true;
        }
    }
    //////////
    APlist.push_back(newAP);
    // Serial.println("Add new AP");
    log_i("[WIFI][APlistAdd] add SSID: %s", newAP.ssid);
    return true;
}
String WiFiMulti::APdataString()
{
    String wifi_data_str = "";
    for(uint8_t i = 0; i < APlist.size(); i++) 
    {
        WifiAPlist_t entry = APlist[i];
        wifi_data_str += String(entry.ssid);
        wifi_data_str += String("\r\n");
        wifi_data_str += String(entry.passphrase);
        wifi_data_str += String("\r\n");
    }
    // Serial.println("WiFi AP info: \n" + wifi_data_str);
    return wifi_data_str;
}
...
void WiFiMulti::removeWiFiNoInternet(String _current_ssid)
{
    for(uint8_t i = 0; i < APlist.size(); i++) 
    {
        if(_current_ssid == String(APlist[i].ssid))
        {
